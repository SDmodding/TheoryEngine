...
namespace pugi
{
	...
	{
	public:
		xml_allocator(xml_memory_block* root, u64 alloc_params, UFG::qMemoryPool* pool): _root(root), _alloc(alloc_params), _pool(pool)
		{
		}
	...
	private:
		...
		void* memalloc(size_t size)
		{
			...
			{
				_root->next = new (_pool->Allocate(sizeof(xml_memory_block), "pugi.xml_memory_block", _alloc)) xml_memory_block();
				_root = _root->next;
	...
	struct xml_attribute_struct
	{
		...
		void free()
		{
			if (!name_insitu) UFG::qFree(name);
			if (!value_insitu) UFG::qFree(value);
		}
	...
	{
		xml_document_struct(): xml_node_struct(node_document), allocator(0, 0, 0)
		{
		}
...
{	
	...
	bool strcpy_insitu(char*& dest, bool& insitu, const char* source)
	{
		...
		{
			char* buf = new char[source_size + 1];
			strcpy(buf, source);
	...
	struct xml_parser
	{
		...
		xml_parser(xml_allocator& alloc, const char* filename): alloc(alloc), _filename(filename)
		{
		}
		
		bool parse(char* s, xml_node_struct* xmldoc, unsigned int optmsk = parse_default, UFG::qString* error_string = 0)
		{
				...
				LOC_TAG:
					...
					else if (*s == '/')
					{
						...
						if (*name)
						{
							if (error_string)
							{
								const char* filename = (_filename ? _filename : "unknown filename, maybe a generated buffer?");
								auto parent = cursor->parent;
								const char* parentname = (parent ? parent->name : "(no parent node)");
...
namespace pugi
{
	...
	xml_document::xml_document(UFG::qMemoryPool* pool, u64 alloc_params) : _filename(0), _buffer(0), _pool(pool), _alloc(alloc_params)
	{
		create();
	}
	...
	{
		xml_allocator alloc(&_memory, _alloc, _pool);
	...
	xml_document::LOAD_RESULT xml_document::load(const char* contents, int length, unsigned int options)
	{
		int len = (length == -1 ? UFG::qStringLength(contents) : length);

		auto xml = static_cast<char*>(_pool->Allocate(len + 1, "pugi.load"));
		if (!xml) {
			return LOAD_OUT_OF_MEMORY;
		}

		UFG::qMemCopy(xml, contents, len);
		xml[len] = 0;

		free();

		xml_allocator& alloc = static_cast<xml_document_struct*>(_root)->allocator;
		xml_parser parser(alloc, _filename);

		if (!parser.parse(xml, _root, options))
		{
			_pool->Free(xml);
	...
	xml_document::LOAD_RESULT xml_document::load_file(const char* name, unsigned int options, UFG::qString* errorString)
	{
		// TODO: Uncomment this when `XMLCache` is implemented.
		char* xml = 0; // SimpleXML::XMLCache::ExtractFromCache(name);

		if (!xml)
		{
			_filename = name;

			auto file = UFG::qOpen(name, UFG::QACCESS_READ, 1);
			if (!file) {
				return LOAD_FILE_DOES_NOT_EXIST;
			}

			s64 len = UFG::qGetFileSize(file);
			xml = static_cast<char*>(_pool->Allocate(len + 1, "pugi.load"));
			if (!xml)
			{
				UFG::qClose(file);
				return LOAD_OUT_OF_MEMORY;
			...
			s64 readLen = UFG::qRead(file, xml, len);
			UFG::qClose(file);
		...
		if (!parser.parse(xml, _root, options, errorString))
		{
			_pool->Free(xml);
			return LOAD_PARSE_ERROR;
		}
